...
from catalog.tests.conftest import BookFactory
...
def test_out_of_stock_item_cannot_be_added_flow(client, user):
    ...
    response = client.get(
        reverse("catalog:cart_add", kwargs={"book_id": out_of_stock_book.id}),
        follow=True,
    )
    assert response.status_code == 200
...
def test_async_views_integration_flow(client, book, order):
    ...
    res3 = client.get(
        reverse("catalog:async_order_status", kwargs={"order_id": order.id})
    )
